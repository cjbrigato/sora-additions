        ...
        try {
            switch (msg.type) {
                case 'SET_TOKEN': {
                    bearer = msg.token;
                    try {
                        await chrome.storage.session.set({ bearer });
                    }
                    catch { }
...
async function ensureBearer() {
    ...
    if (!bearer)
        throw new Error('No token captured yet');
}
...
function startDirect(tabId, items, parallel, saveAs) {
    ...
    while (currentBatch.active < currentBatch.max)
        dequeueNext();
}
...
function dequeueNext() {
    ...
    chrome.downloads.download({ url: item.url, filename: item.filename, conflictAction: 'uniquify', saveAs: currentBatch.saveAs }, (dlId) => {
        if (!dlId) {
            currentBatch.active -= 1;

...
(() => {
    ...
    async function downloadAllToOPFS({ items, onStatus, signal }) {
        ...
        for (const it of items) {
            ...
            if (!res.ok || !res.body) {
                await w.close();
                throw new Error(`Fetch failed: ${safeName}`);
            }
        ...
        return new Promise((resolve) => {
            chrome.storage.sync.get('soraDownloaderSettings', (data) => {
                try {
                    ...
                    if (saved.directMaxItems && !saved.directMaxTasks)
                        saved.directMaxTasks = saved.directMaxItems;
      ...
      `;
        ...
        function setPanelProgress(pct, main, sub) {
            ...
            if (typeof pct === 'number')
                ringEl.style.setProperty('--pct', `${Math.max(0, Math.min(100, pct))}%`);
        ...
        function setMiniBadge(text, phase) {
            ...
            if (phase === 'dl')
                mini.classList.add('dl');
            else if (phase === 'zip')
                mini.classList.add('zip');
        ...
        function updateSettingsUI() {
            ...
            if (!canNoWM) {
                ui.modeFinalRadio && (ui.modeFinalRadio.disabled = true);
                currentSettings.fastDownload = true;
            }
            else {
                ui.modeFinalRadio && (ui.modeFinalRadio.disabled = false);
            }
        ...
        ui.copyBtn?.addEventListener('click', async () => {
            try {
                await navigator.clipboard.writeText(ui.resultTA.value);
            }
            catch {
        ...
        ui.exportBtn?.addEventListener('click', () => {
            ...
            const csvRows = [csvHeader.join(',')].concat(lastManifest.rows.map(r => [r.id, toCSV(r.filename), toCSV(r.url), lastManifest.mode, lastManifest.quality].join(',')));
        ...
        ui.runBtn?.addEventListener('click', async () => {
            ...
            try {
                ...
                if (!resList?.ok)
                    throw new Error(resList?.error || 'List fetch failed');
                ...
                if (valid.length) {
                    ...
                    if (doDirect) {
                        ...
                        if (currentSettings.directZip) {
                            ...
                            const acZIP = new AbortController();
                            currentAbort = acZIP;
                            ...
                            // Cleanup OPFS
                            try {
                                await opfsRemoveDir(root, dirName);
                            }
                            catch { }
                            currentAbort = null;
                            directRunning = false;
                    ...
                    if (totalSkipped > 0)
                        finalStatus += ` (${totalSkipped} skipped/failed).`;
                    if (doDirect)
                        finalStatus += ` Direct mode used for ${validTasksCount} task(s).`;
        ...
        for (const task of tasks) {
            ...
            for (const gen of gens) {
                ...
                if (ok) {
                    ...
                    skipped.push({ id: gen?.id || task?.id, reason: 'Missing video file (encodings)' });
                }
            }
        }
    ...
    function countValidTasks(tasks) {
        ...
        for (const t of tasks) {
            ...
            for (const g of gens) {
                const e = g?.encodings;
                if (e?.source?.path || e?.md?.path || e?.ld?.path) {
                    ok = true;
                    break;
                }
            }
    ...
    async function fetchRawWithConcurrency(ids, concurrency, onProgress) {
        ...
        async function worker() {
            while (queue.length) {
                ...
                if (res?.ok && res.url)
                    successes.push({ id, url: res.url });
                else
                    failures.push({ id, reason: res?.error || 'Unknown error' });
                processed++;
    ...
    function generateScript(downloadRows, mode, quality, skipped, failures, dryRun) {
        ...
        if (skipped.length) {
            ...
            for (const s of skipped)
                blocks.push(`# ${s.id}: ${s.reason}`);
            blocks.push('');
        }
        if (failures.length) {
            blocks.push(`# --- FAILED during URL fetch ---`);
            for (const f of failures)
                blocks.push(`# ${f.id}: ${f.reason}`);
            blocks.push('');
        }

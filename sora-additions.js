// ==UserScript==
// @name         Sora Batch Downloader (Generator) v0.3.1 — Shadow DOM
// @namespace    https://github.com/cjbrigato/tampermonkey-sora-additions
// @version      0.3.1
// @description  Generate robust bash scripts to download your Sora videos in batch (self-service only, no scraping beyond your session). Shadow DOM UI (same skin as 0.3.0).
...
(function() {
    ...
    // Voluntary cap at 100; can be reduced via settings, never increased.
    ...
    const DEFAULT_SETTINGS = {
      workers: 8,                   // concurrent /raw (final mode only)
      fastDownload: false,          // fast = watermark (encodings source/md/ld)
      fastDownloadQuality: 'source',// 'source' | 'md' | 'ld'
      limit: DEFAULT_LIMIT,         // capped at 100
      dryRun: false                 // comment out curls for inspection
    };
    let currentSettings = { ...DEFAULT_SETTINGS };
  
    // UI registry (filled after Shadow DOM mount)
    const ui = {
      root: null,
      launcher: null,
      launcherBorder: null,
      ...
      limitInput: null,
      dryRunCheckbox: null,
      finalQualityOption: null,
      fastQualityContainer: null,
      parallelContainer: null,
      updateSettingsUI: null,
      populateSettingsPanel: null,
      toggleSettingsInteractivity: null,
    };
    ...
    async function initializeApp() {
      ...
      try {
        ...
        // If UI already mounted, switch view immediately
        renderAppView();
      } catch (err) {
        if (ui.statusDiv) ui.statusDiv.textContent = 'Error loading user permissions.';
      }
    }
    ...
    // Global view toggles (safe if UI not yet mounted)
    ...
    function renderAppView() {
      if (!ui.panel || !isAppInitialized) return;
    ...
    function injectShadowUI() {
      ...
      // Styles + Markup (same skin as v0.3.0, but local to shadow)
      root.innerHTML = `
        <style>
      ...
      // Local helpers (kept identical to 0.3.0 behavior)
      ui.updateSettingsUI = function updateSettingsUI() {
        ...
        ui.populateSettingsPanel();
      };
      ui.populateSettingsPanel = function populateSettingsPanel() {
        ...
        ui.toggleSettingsInteractivity(currentSettings.fastDownload);
      };
      ui.toggleSettingsInteractivity = function toggleSettingsInteractivity(isFast) {
      ...
      root.getElementById('sora-download-mode-selector').addEventListener('change', (e) => {
        if (e.target.name === 'sora-download-mode') {
          ui.toggleSettingsInteractivity(e.target.value === 'fast');
        }
      });
      ...
      ui.settingsSave.addEventListener('click', async () => {
        ...
        currentSettings.limit = Math.min(Math.max(1, parseInt(ui.limitInput.value,10) || DEFAULT_SETTINGS.limit), DEFAULT_SETTINGS.limit);
        currentSettings.dryRun = !!ui.dryRunCheckbox.checked;
        await saveSettings();
        ui.settingsPanel.style.display = 'none';
      });
      ui.copyBtn.addEventListener('click', () => {
        GM_setClipboard(ui.resultTA.value);
        ui.copyBtn.textContent = 'Copied!';
        setTimeout(() => { ui.copyBtn.textContent = 'Copy Script'; }, 1800);
      });
  
      GM_registerMenuCommand?.('Open Sora Downloader', () => {
        ui.panel.style.display = 'flex'; ui.launcher.style.display = 'none';
        isAppInitialized ? renderAppView() : renderNoTokenView();
      });
      ...
      ui.runBtn.addEventListener('click', async () => {
        ...
        // Spinner/Progress ring (same vibe)
        ...
        try {
          ...
          ui.resultTA.value = `An error occurred. Check the console (F12).\n\n${err.stack || String(err)}`;
        } finally {
      ...
      // If token already captured, reflect correct view
      isAppInitialized ? renderAppView() : renderNoTokenView();
    }
      ...
      async function worker() {
        while (queue.length) {
          ...
          try {
            ...
            if (ui.statusDiv) ui.statusDiv.textContent = txt + '...';
            onProgress?.(txt, pct);
          }
        }
      }
    ...
    function fileNameFor(id) {
      return `sora_${id}.mp4`; // stable & future-proof
    }
  
    function generateScript(downloadRows, mode, quality, skipped, failures, dryRun) {
      if (ui.statusDiv) ui.statusDiv.textContent = 'Step 3/3: Generating final script...';
